...
class AndroidAssetManager(val context: Context) : AssetManager() {
    override var assetsBaseDir: String = "."
    ...
    override fun loadAsset(assetPath: String, onLoad: (ByteArray) -> Unit) {
        launch {
            ...
            this@AndroidAssetManager.context.assets.open(path)?.use {
                ...
                Log.d("KoolActivity", "Loaded asset \"$path\" in ${(System.nanoTime() - t) / 1e6} ms (${bytes.size / (1024.0*1024.0)} MB)")
