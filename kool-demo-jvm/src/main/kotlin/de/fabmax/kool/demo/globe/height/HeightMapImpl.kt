...
actual fun loadPngS16HeightMap(basePath: String, meta: HeightMapMeta): BoundedHeightMap {
    val img = ImageIO.read(File(basePath, meta.path))
    if (img.width != meta.width || img.height != meta.height) {
        throw IllegalStateException("Image size differs from meta info: img: ${img.width} x ${img.height}, meta: ${meta.width} x ${meta.height} [${meta.path}]")
    }
    ...
    if (img.type == BufferedImage.TYPE_INT_ARGB || img.type == BufferedImage.TYPE_INT_RGB) {
        ...
        throw NotImplementedError("Unsupported image type: ${img.type} [${meta.path}]")
    }

    return HeightMapS16(data, meta)
}