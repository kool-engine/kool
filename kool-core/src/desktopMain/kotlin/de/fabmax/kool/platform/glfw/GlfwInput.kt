...
class GlfwInput(val window: GlfwWindow) : PlatformInput {
    ...
    internal fun onContextCreated(ctx: Lwjgl3Context) {
        ...
        installInputHandlers(window.windowHandle)
    ...
    private fun installInputHandlers(windowHandle: Long) {
        ...
        glfwSetCursorEnterCallback(windowHandle) { _, entered ->
            if (!entered) {
                window.isMouseOverWindow = false
                PointerInput.handleMouseExit()
            } else {
                window.isMouseOverWindow = true
            }
        }
        ...
        for (i in GLFW_JOYSTICK_1 until GLFW_JOYSTICK_LAST) {
            if (glfwJoystickPresent(i)) {
                ControllerInput.addController(GlfwController(i))
            }
        }

        glfwSetJoystickCallback { joystickId, event ->
            when (event) {
                GLFW_CONNECTED -> ControllerInput.addController(GlfwController(joystickId))
