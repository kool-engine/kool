...
expect fun getGamepadState(index: Int): Gamepad?

class ControllerWeb(private var gamepad: Gamepad) : Controller(gamepad.index) {
    ...
    init {
        if (isGamepad) {
            ...
            buttonStates = BooleanArray(gamepad.buttons.length)
            axisStates = FloatArray(gamepad.axes.length)
        }
        logI { "Controller connected: $name (${gamepad.axes.length} axes, ${gamepad.buttons.length} buttons, isGamepad: $isGamepad)" }
    }

    override fun updateState() {
        gamepad = getGamepadState(gamepad.index) ?: return
        if (!isConnected) {
            return
        }
        ...
        if (isGamepad) {
            ...
            for (i in 0 until min(axisStates.size, gamepad.axes.length)) {
    ...
    private fun Gamepad.getButtonState(index: Int): Boolean {
        ...
        return buttons[index]?.pressed == true
    }
