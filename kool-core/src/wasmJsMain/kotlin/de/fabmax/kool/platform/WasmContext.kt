...
suspend fun WasmContext(): WasmContext {
    val config = KoolSystem.configWasm
    ...
    val ctx = WasmContext(canvas, config)
    ctx.createBackend()
    return ctx
}
...
class WasmContext internal constructor(canvas: HTMLCanvasElement, val config: KoolConfigWasm) : KoolContext() {
    override lateinit var backend: RenderBackend
        private set

    override val window: WasmWindow = WasmWindow(canvas, config)

    private val sysInfo = mutableListOf<String>()

    internal suspend fun createBackend() {
        val backendProvider = config.renderBackend
        val backendResult = backendProvider.createBackend(this@WasmContext)
        backend = when {
...
class WasmWindow(val canvas: HTMLCanvasElement, val config: KoolConfigWasm) : KoolWindow {
    ...
    override var sizeOnScreen: Vec2i
        ...
        set(value) {
            logE { "WasmWindow size cannot be set" }
        }
