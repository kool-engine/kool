...
class FontMapGenerator(val maxWidth: Int, val maxHeight: Int) {
    ...
    init {
        val pixelRatio = (KoolSystem.getContextOrNull() as WasmContext?)?.window?.parentScreenScale ?: 1f
        ...
        KoolSystem.configWasm.customTtfFonts.forEach { (family, url) ->
            loadFont(family, url)
        }
    }
    ...
    fun createFontMapData(font: AtlasFont, fontScale: Float, outMetrics: MutableMap<Char, CharMetrics>): BufferedImageData2d {
        ...
        // clear canvas
        canvasCtx.fillStyle = "#000000".toJsString()
    ...
    private fun makeMap(font: AtlasFont, size: Int, outMetrics: MutableMap<Char, CharMetrics>): Int {
        ...
        canvasCtx.fillStyle = "#ffffff".toJsString()
        canvasCtx.strokeStyle = "#ffffff".toJsString()
...
external class FontFace(family: String, source: String) : JsAny {
    val family: String

    fun load(): Promise<FontFace>
}

private fun addFont(f: FontFace): JsAny = js("document.fonts.add(f)")
