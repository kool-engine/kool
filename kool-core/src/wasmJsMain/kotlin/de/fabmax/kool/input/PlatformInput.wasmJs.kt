...
internal actual fun PlatformInput(): PlatformInput = PlatformInputWasm

internal object PlatformInputWasm : PlatformInput {

    val excludedKeyCodes = mutableSetOf("F5", "F11", "F12", "MetaLeft", "MetaRight")

    private val pixelRatio: Float
        get() = (KoolSystem.getContextOrNull() as WasmContext?)?.window?.renderScale ?: 1f
    ...
    override fun applyCursorShape(cursorShape: CursorShape) {
        if (cursorShape != currentCursorShape) {
            WasmImpl.canvas.style.cursor = when (cursorShape) {
    ...
    fun onContextCreated(ctx: WasmContext) {
    ...
    private fun installInputHandlers(canvas: HTMLCanvasElement) {
        ...
        navigator.getGamepads()?.let { gamepads ->
            for (i in 0 until gamepads.length) {
                gamepads[i]?.let { ControllerInput.addController(ControllerWeb(it)) }
            }
        }

        if (KoolSystem.configWasm.isGlobalKeyEventGrabbing) {
    ...
    private fun installMouseHandlers(canvas: HTMLCanvasElement) {
        ...
        canvas.onmousemove = { ev ->
            ...
            if (PointerLockState.hasPointerLock) {
                ...
                virtualPointerPos.x += getMovementX(ev).toDouble() * pixelRatio
                virtualPointerPos.y += getMovementY(ev).toDouble() * pixelRatio
            } else {
    ...
    private fun installTouchHandlers(canvas: HTMLCanvasElement) {
        ...
        canvas.addEventListener("touchstart", { ev ->
            ...
            for (i in 0 until changedTouches.length) {
                changedTouches.item(i)?.let { touch ->
    ...
    private object PointerLockState {
        ...
        var cursorMode = CursorMode.NORMAL
            set(value) {
                ...
                when (value) {
                    ...
                    CursorMode.LOCKED -> requestPointerLock(WasmImpl.canvas)
                }
            }
        ...
        fun onPointerLockChange(canvas: HTMLCanvasElement) {
            hasPointerLock = hasCanvasPointerLock(canvas).toBoolean()
        ...
        fun checkLockState() {
            if (cursorMode == CursorMode.LOCKED && !hasPointerLock) {
                ...
                requestPointerLock(WasmImpl.canvas)
            }
        }
    }
}
...
private fun getMovementY(ev: MouseEvent): JsNumber = js("ev.movementY")
private fun requestCanvasPointerLock(canvas: HTMLCanvasElement): JsAny = js("canvas.requestPointerLock()")
private fun exitCanvasPointerLock(): JsAny = js("document.exitPointerLock()")
private fun hasCanvasPointerLock(canvas: HTMLCanvasElement): JsBoolean = js("document.pointerLockElement === canvas")