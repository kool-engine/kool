...
import de.fabmax.kool.toIntArray
import de.fabmax.kool.toJsArray
import de.fabmax.kool.toUint32Array
...
object GlImpl : GlApi {
    ...
    override val TRUE = true.toJsBoolean()
    ...
    override fun drawBuffers(buffers: IntArray) = gl.drawBuffers(buffers.toUint32Array())
    ...
    override fun getActiveUniforms(program: GlProgram, uniformIndices: IntArray, pName: Int): IntArray = gl.getActiveUniforms(program.webGl, uniformIndices.toUint32Array(), pName).toIntArray()
    override fun getError(): Int = gl.getError()
    override fun getInteger(pName: Int): Int = (gl.getParameter(pName) as JsNumber).toInt()
    ...
    override fun getUniformIndices(program: GlProgram, names: Array<String>): IntArray = gl.getUniformIndices(program.webGl, names.toJsArray()).toIntArray()
    ...
    fun initWebGl(glCtx: WebGL2RenderingContext) {
        ...
        var maxAnisotropy = 1
        val extAnisotropic: EXT_texture_filter_anisotropic? =
            gl.getExtension("EXT_texture_filter_anisotropic")?.unsafeCast<EXT_texture_filter_anisotropic>() ?:
            gl.getExtension("MOZ_EXT_texture_filter_anisotropic")?.unsafeCast<EXT_texture_filter_anisotropic>() ?:
            gl.getExtension("WEBKIT_EXT_texture_filter_anisotropic")?.unsafeCast<EXT_texture_filter_anisotropic>()
        if (extAnisotropic != null) {
            TEXTURE_MAX_ANISOTROPY_EXT = extAnisotropic.TEXTURE_MAX_ANISOTROPY_EXT
            maxAnisotropy = (gl.getParameter(extAnisotropic.MAX_TEXTURE_MAX_ANISOTROPY_EXT) as JsNumber).toInt()
        }

        clipControlExt = gl.getExtension("EXT_clip_control")?.unsafeCast<EXT_clip_control>()
        clipControlExt?.let {
        ...
        val maxTexUnits = (gl.getParameter(WebGLRenderingContext.MAX_TEXTURE_IMAGE_UNITS) as JsNumber).toInt()
    ...
    override fun readTexturePixels(src: LoadedTextureGl, dst: BufferedImageData): Boolean {
        ...
        if (checkFramebufferStatus(FRAMEBUFFER) == FRAMEBUFFER_COMPLETE) {
            val format = gl.getParameter(WebGLRenderingContext.IMPLEMENTATION_COLOR_READ_FORMAT) as JsNumber
            val type = gl.getParameter(WebGLRenderingContext.IMPLEMENTATION_COLOR_READ_TYPE) as JsNumber
            gl.readPixels(0, 0, src.width, src.height, format.toInt(), type.toInt(), dst.arrayBufferView)
        } else {
