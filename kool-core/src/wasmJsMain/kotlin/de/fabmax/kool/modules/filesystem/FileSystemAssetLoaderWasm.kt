...
class FileSystemAssetLoaderWasm(baseDir: FileSystemDirectory) : FileSystemAssetLoader(baseDir) {
    override suspend fun loadImageAtlas(ref: AssetRef.ImageAtlas): LoadedAsset.ImageAtlas {
        val result = loadData(ref.path).mapCatching { texData ->
            ...
            parts[0] = bytes
            val imgBlob = Blob(parts)
            val bmp: ImageBitmap = window.createImageBitmap(imgBlob, ImageBitmapOptions(ref.resolveSize)).await()
