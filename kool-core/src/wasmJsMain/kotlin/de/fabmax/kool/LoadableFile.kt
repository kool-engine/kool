...
class LoadableFileImpl(val file: File) : LoadableFile {
    ...
    override suspend fun read(): Uint8Buffer {
        val data = getBlobArrayBuffer(file)
        return Uint8BufferImpl(Uint8Array(data.await<ArrayBuffer>()))
    }
...
private fun getBlobArrayBuffer(blob: Blob): Promise<ArrayBuffer> = js("blob.arrayBuffer()")
