...
fun atmosphereData(): AtmosphereData {
    return (program.dataBlocks.find { it is AtmosphereData } as? AtmosphereData) ?: AtmosphereData(program)
}

class AtmosphereData(program: KslProgram) : KslDataBlock(NAME, program) {
    val uDirToSun = uniformFloat3("uDirToSun")
    val uPlanetCenter = uniformFloat3("uPlanetCenter")
    val uSurfaceRadius = uniformFloat1("uSurfaceRadius")
    val uAtmosphereRadius = uniformFloat1("uAtmosphereRadius")
    val uScatteringCoeffs = uniformFloat3("uScatteringCoeffs")
    val uMieG = uniformFloat1("uMieG")
    val uMieColor = uniformFloat4("uMieColor")
    val uRayleighColor = uniformFloat4("uRayleighColor")
    val uSunColor = uniformFloat4("uSunColor")
    val uRandomOffset = uniformFloat2("uAtmoRandOffset")
...
class AtmosphereBlock(
    ...
    val inSceneColor = inFloat4("inSceneColor", float4Value(0f, 0f, 0f, 1f))
    val inSkyColor = inFloat4("inSkyColor", float4Value(0f, 0f, 0f, 1f))
    val inSceneDepth = inFloat1("inSceneDepth", 0f.const)
    val inViewDepth = inFloat1("inViewDepth", 0f.const)
    val inScenePos = inFloat3("inScenePos", float3Value(0f, 0f, 0f))
    val inCamPos = inFloat3("inCamPos", float3Value(0f, 0f, 0f))
    val inLookDir = inFloat3("inLookDir", float3Value(0f, 0f, 1f))

    val outColor = outFloat4("outColor")

    private val funPhaseFunRayleigh = context(parentScope.parentStage) {
        functionFloat3("phaseFunRayleigh") {
    ...
    private val funPhaseFunMie = context(parentScope.parentStage) {
        functionFloat3("phaseFunMie") {
            val cosTheta = paramFloat1("cosTheta")
            val g = paramFloat1("g")
            body {
    ...
    private val funOpticalDepth = context(parentScope.parentStage) {
        functionFloat2("opticalDepth") {
            val altitude = paramFloat1("altitude")
            val cosTheta = paramFloat1("cosTheta")
            body {
                opticalDepthLut.sample(float2Value(cosTheta * 0.5f.const + 0.5f.const, altitude), 0f.const).rg
            }
        }
    }

    private val funOpticalDepthLen = context(parentScope.parentStage) {
        functionFloat1("opticalDepthLen") {
    ...
    private val funScatterLight = context(parentScope.parentStage) {
        functionFloat3("scatterLight") {
            ...
            body {
                ...
                fori(0.const, numScatterSamples.const) { i ->
                    ...
                    val shadowFac = if (sunShadowProj != null && sunShadowTex != null) {
                        ...
                        float1Var(sunShadowTex.sample(shadowCoord.xy, shadowCoord.z))
                    } else {
                        1f.const
                    }
