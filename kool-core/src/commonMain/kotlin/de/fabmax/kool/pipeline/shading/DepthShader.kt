            ...
            fun forMesh(
                ...
                vertices {
                    if (mesh.instances != null) {
                        val modelMat = mesh.instances.layout.getByName(InstanceLayouts.ModelMat.modelMat.name)
                        if (modelMat is Mat4Member) {
                            instancedModelMatrix(modelMat)
                        } else {
                            logE { "Mesh ${mesh.name} has instance attributes, but does not contain an instanced model matrix member name ${InstanceLayouts.ModelMat.modelMat.name}" }
                        }
                    }
                    mesh.skin?.let {
