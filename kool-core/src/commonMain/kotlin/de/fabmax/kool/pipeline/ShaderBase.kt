...
abstract class ShaderBase<T: PipelineBase>(val name: String) {
    ...
    fun bindStorage(storageName: String, defaultVal: GpuBuffer? = null): StorageBufferBinding =
        getOrCreateBinding(storageName) { StorageBufferBinding(storageName, defaultVal, this) } as StorageBufferBinding

    fun bindStorageTexture1d(storageTextureName: String, defaultVal: StorageTexture1d? = null, defaultMipLevel: Int = 0): StorageTexture1dBinding =
    ...
    fun bindStorageTexture2d(storageTextureName: String, defaultVal: StorageTexture2d? = null, defaultMipLevel: Int = 0): StorageTexture2dBinding =
    ...
    fun bindStorageTexture3d(storageTextureName: String, defaultVal: StorageTexture3d? = null, defaultMipLevel: Int = 0): StorageTexture3dBinding =
    ...
    fun bindColorUniform(cfg: ColorBlockConfig): UniformBindingColor =
        bindUniformColor(cfg.primaryUniform?.uniformName ?: UniqueId.nextId("_"), cfg.primaryUniform?.defaultColor ?: Color.BLACK)
    fun bindColorTexture(cfg: ColorBlockConfig): Texture2dBinding =
        bindTexture2d(cfg.primaryTexture?.textureName ?: UniqueId.nextId("_"), cfg.primaryTexture?.defaultTexture)

    fun bindNormalTexture(cfg: NormalMapConfig): Texture2dBinding =
        if (cfg.isNormalMapped && !cfg.isArrayNormalMap) bindTexture2d(cfg.textureName, cfg.defaultNormalMap) else Texture2dBinding("-", null, null, this)

    fun bindPropertyUniform(cfg: PropertyBlockConfig): UniformBinding1f =
        bindUniformFloat1(cfg.primaryUniform?.uniformName ?: UniqueId.nextId("_"), cfg.primaryUniform?.defaultValue ?: 0f)
    fun bindPropertyTexture(cfg: PropertyBlockConfig): Texture2dBinding =
        bindTexture2d(cfg.primaryTexture?.textureName ?: UniqueId.nextId("_"), cfg.primaryTexture?.defaultTexture)
