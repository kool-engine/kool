...
class VehicleAudio(physicsWorld: PhysicsWorld) {
    ...
    suspend fun loadAudio(assetMgr: Assets) {
        assetMgr.apply {
            ...
            val idle1 = WavNode(WavFile(loadBlobAsset("${DemoLoader.soundPath}/car/rpm_idle1.wav").getOrThrow()))
            val idle2 = WavNode(WavFile(loadBlobAsset("${DemoLoader.soundPath}/car/rpm_idle2.wav").getOrThrow()))
            val idleMix = ModulateNode(idle1, idle2).apply { gain = 1.5f }

            val mid1 = WavNode(WavFile(loadBlobAsset("${DemoLoader.soundPath}/car/rpm_mid1.wav").getOrThrow()))
            val mid2 = WavNode(WavFile(loadBlobAsset("${DemoLoader.soundPath}/car/rpm_mid2.wav").getOrThrow()))
            ...
            val high1 = WavNode(WavFile(loadBlobAsset("${DemoLoader.soundPath}/car/rpm_high1.wav").getOrThrow()))
            val high2 = WavNode(WavFile(loadBlobAsset("${DemoLoader.soundPath}/car/rpm_high2.wav").getOrThrow())).apply { speed = 1.05f }
            ...
            val squeal1 = WavNode(WavFile(loadBlobAsset("${DemoLoader.soundPath}/car/squeal1.wav").getOrThrow()))
            val squeal2 = WavNode(WavFile(loadBlobAsset("${DemoLoader.soundPath}/car/squeal2.wav").getOrThrow()))
            squealMix = SquealModulateNode(squeal1, squeal2)

            gearShiftNode = WavNode(WavFile(loadBlobAsset("${DemoLoader.soundPath}/car/gear_shift.wav").getOrThrow())).apply { loop = false }
