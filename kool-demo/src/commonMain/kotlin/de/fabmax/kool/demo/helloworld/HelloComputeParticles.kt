    ...
    fun particleComputeShader() = KslComputeShader("particle compute shader") {
        ...
        computeStage(workGroupSizeX = 64) {
            main {
                ...
                val particle = particleVar
                val lifeTime = float1Var(particle[Particle.lifeTime] - deltaT)
                ...
                `if`(lifeTime le 0f.const) {
                    ...
                    particle[Particle.appearance][ParticleAppearance.color] set gradient.load((abs(a - 1f.const) * 255f.const).toInt1(), 0.const)
                    particle[Particle.position] set float3Value(cos(a * PI_F.const), 0f.const, sin(a * PI_F.const)) * r
                    particle[Particle.velocity] set (noise13(seed + 4711f.const) + float3Value(-0.5f, 1f, -0.5f)) * 2f.const
                    lifeTime set 0.75f.const + noise11(seed)
                }

                // update particle attributes
                particle[Particle.velocity].y -= 10f.const * deltaT
                particle[Particle.position] += particle[Particle.velocity] * deltaT
                particle[Particle.appearance][ParticleAppearance.size] set 5f.const * sqrt(lifeTime)
                particle[Particle.lifeTime] set lifeTime
    ...
    fun particleDrawShader() = KslShader("particle draw shader") {
        ...
        vertexStage {
            main {
                ...
                val particleData = structVar(particleBuffer[inVertexIndex.toInt1()])
                val cam = cameraData()
                val modelMat = modelMatrix()

                outPosition set cam.viewProjMat * modelMat.matrix * float4Value(particleData[Particle.position], 1f.const)
                outPointSize set particleData[Particle.appearance][ParticleAppearance.size]
        ...
        fragmentStage {
            main {
                ...
                colorOutput(particleBuffer[index.output][Particle.appearance][ParticleAppearance.color])
            }
        }
    }
